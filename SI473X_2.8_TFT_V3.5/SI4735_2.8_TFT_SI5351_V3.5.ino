//  V3.5   30-01-2022 Under #ifdef conditions horizontal display plus 180 degrees added.
//  V3.5   30-01-2022 Second bandplan added under #ifdef conditions. Europe & Asia and North & South America. (EuroAsiabandplan & NandSAmericanbandplan)
//  V3.5   30-01-2022 Two bugs solved. Problem with the BFO frequentie in Crystal mode and a display problem after wake up of the display.
//  V3.5   15-12-2021 Lipo status bar added under #ifdef conditions. (IhaveLiPoLoadbar)
//  V3.4   24-11-2021 Bug support.
//  V3.4   24-11-2021 Memory added for BFO for each band when crystal is in use.
//  V3.1   29-09-2021 Added Sprite buttons from Jim Yasuda.
...
//  included.
//  Also a schematic drawing is available. See github pe0mgb.

//  ABOUT SSB PATCH:
//  First of all, it is important to say that the SSB patch content is not part of this library. The patches used here were made available by Mr.
...
//  The author of this sketch does not know the seller of this kit and does not have a commercial relationship with any commercial product that uses the Arduino Library.
...
//  Library Rotary is provided with the program     : http://www.buxtronix.net/2011/10/rotary-encoders-done-properly.html
//  Library SI4735 you may download from here       : https://github.com/pu2clr/SI4735
//  This sketch and other information from here     : https://github.com/pe0mgb//SI4735-Radio-ESP32-Touchscreen-Arduino
//
//  *********************************
...
uint16_t Xbut8  = 3 * Xbutsiz ; int      Ybut8  = -4 * Ybutsiz;
uint16_t Xbut9  = 3 * Xbutsiz ; int      Ybut9  = -3 * Ybutsiz;
uint16_t Xbut10 = 3 * Xbutsiz ; int      Ybut10 = -2 * Ybutsiz;
uint16_t Xbut11 = 3 * Xbutsiz ; int      Ybut11 = -1 * Ybutsiz;
#endif
...
void showRSSI() {
  ...
  if ((  currentMode == FM ) and ((FirstLayer) or (ThirdLayer))) {
    ...
    tft.fillRect(XFreqDispl + 191, YFreqDispl + 72 , 38, 11, TFT_RED); // STEREO MONO
    tft.drawString(buffer, XFreqDispl + 210, YFreqDispl + 83);
  }
  rssi = NewRSSI;
  ...
  if ((FirstLayer) or (ThirdLayer)) {  // dBuV and dB at freq. display
    ...
    tft.drawString("RSSI = " + String(NewRSSI) + " dBuV" , XFreqDispl + 8, YFreqDispl + 76);
    tft.setTextDatum(TR_DATUM);
    tft.drawString("SNR = " + String(NewSNR) + " dB", XFreqDispl + 180, YFreqDispl + 76);
  }
  VOLbutoff();
}
...
void SeekFreq (uint16_t freq)  {
  ...
  if ((FirstLayer) or (ThirdLayer))  {
    ...
    tft.setTextDatum(BC_DATUM);
    tft.setTextPadding(0);
    tft.fillRect( XFreqDispl + 26, YFreqDispl + 28 , 208, 32, TFT_BLACK); // Black freq. field
    if (band[bandIdx].bandType == MW_BAND_TYPE || band[bandIdx].bandType == LW_BAND_TYPE) {
      Displayfreq =  currentFrequency;
      tft.setTextSize(1);
      tft.setTextColor(COLOR_INDICATOR_FREQ, COLOR_BACKGROUND);
...
void DrawDispl() {
  //=======================================================================================
  if (FirstLayer or ThirdLayer) {
    tft.fillRect(XFreqDispl + 26, YFreqDispl + 5, 208, 80, TFT_BLACK);
    tft.setTextSize(1);
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.setTextDatum(BC_DATUM);
...
void Lipovolt()
{
  ...
  */
  ...
  if (lipvolt >= 3.9) {
    ...
    tft.fillRect(Xoff + XFreqDispl, 0 + Yoff, 24 , 10 , TFT_GREEN);
    tft.drawString("100%", 12, 9 + Yoff);
  }

  if (lipvolt >= 3.7) {
    //tft.setTextColor(TFT_BLUE, TFT_YELLOW);
    tft.setTextPadding(tft.textWidth("75%"));
    tft.fillRect(Xoff + XFreqDispl, 14 + Yoff, 24 , 10 , TFT_YELLOW);
    //tft.drawString("75%", 12, 23 + Yoff);
  }

  if (lipvolt >= 3.5) {
    //tft.setTextColor(TFT_BLUE, TFT_ORANGE);
    tft.setTextPadding(tft.textWidth("50%"));
    tft.fillRect(Xoff + XFreqDispl, 28 + Yoff, 24 , 10 , TFT_ORANGE);
    //tft.drawString("50%", 12, 37 + Yoff);
  }

  if (lipvolt >= 3.3) {
    //tft.setTextColor(TFT_BLUE, TFT_RED);
    tft.setTextPadding(tft.textWidth("25%"));
    tft.fillRect(Xoff + XFreqDispl, 42 + Yoff, 24 , 10 , TFT_RED);
    //tft.drawString("25%", 12, 51 + Yoff);
  }
  ...
  if ((lipvolt < 3.7) and (lipvolt >= 3.5)) {
    tft.setTextColor(TFT_BLUE, TFT_ORANGE);
    tft.drawString("50%", 12, 37 + Yoff);
  }
  if ((lipvolt < 3.5) and (lipvolt >= 3.3)) {
    tft.setTextColor(TFT_BLUE, TFT_RED);
    tft.drawString("25%", 12, 51 + Yoff);
  }
}
